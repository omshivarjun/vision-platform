...
export default function LoginPage() {
  ...
  const { login, isMicrosoftConfigured } = useAuth()
  ...
  const handleSocialLogin = (provider: string) => {
    if (provider === 'Google') {
      toast.error('Google authentication is currently unavailable. Please use Microsoft login or email/password.')
    } else if (provider === 'GitHub') {
      toast.error('GitHub authentication has been deprecated. Please use Microsoft login or email/password.')
    }
  }

  const handleMicrosoftLoginSuccess = (user: any) => {
    toast.success(`Welcome ${user.firstName}! 🎉`)
    navigate('/dashboard')
  }

  const handleMicrosoftLoginError = (error: any) => {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        >
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Sign in to your Vision Platform account
          </p>
        </motion.div>
          ...
          <div className="mt-6 space-y-3">
            ...
            {isMicrosoftConfigured && (
              <MicrosoftLoginButton
                ...
                onLoginError={handleMicrosoftLoginError}
                className="w-full"
                variant="outline"
                size="md"
              />
            ...
            {/* Google Login Button (Deprecated) */}
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => handleSocialLogin('Google')}
              disabled={true}
              className="w-full inline-flex justify-center py-2 px-4 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm bg-gray-100 dark:bg-gray-600 text-sm font-medium text-gray-400 dark:text-gray-500 cursor-not-allowed opacity-50"
            ...
            {!isMicrosoftConfigured && (
              <div className="text-center p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <p className="text-sm text-yellow-700 dark:text-yellow-300">
                  ⚠️ Microsoft authentication is not configured. Please contact support.
                </p>
              </div>
            )}
          </div>
