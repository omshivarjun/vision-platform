...
router.post('/events', async (req, res) => {
  try {
    ...
    const doc = { name, properties, userId, sessionId, timestamp: new Date() }
    await mongoose.connection.collection('analytics').insertOne(doc as any)
...
router.get('/realtime', async (req, res) => {
  try {
    ...
    const cursor = mongoose.connection.collection('analytics').aggregate([
